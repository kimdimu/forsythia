...
public class Sun2 : MonoBehaviour
{
    ...
    void Start()
    {
        Sun_1Obj = GameObject.Find("Sun_1"); //Sun_1 오브젝트를 찾는다
    ...
    void Update()
    {
        ...
        sun.transform.position = new Vector3(Random.Range(460, 600), SunPosition.transform.position.y,
                                             SunPosition.transform.position.z + 100);
    ...
    void MakeSun()
    {
        IEnRan = Random.Range(2, 5); //생성되는 시간 랜덤으로 지정
        speedRan = Random.Range(40.0f, 100.0f); //스피드 랜덤으로 지정
        sunPran = Random.Range(0, 4);

      
        //Sun_1 물체랑 나의(Sun_2) z좌표 거리가 100보다 크거나 -100보다 작고
        //Sun_3 물체랑 나의(Sun_2) z좌표 거리가 100보다 크거나 -100보다 작으면 생성한다
        if ((Sun_1Obj.transform.position.z - sun.transform.position.z > 100 ||
            Sun_1Obj.transform.position.z - sun.transform.position.z < -100) &&
            ...
            IsClon == false && IsEnd == false)
        {
            //x좌표 랜덤으로 지정, 시작 위치 랜덤으로 지정
            sun.transform.position = new Vector3(Random.Range(460, 600), SunPosition.transform.position.y,
            SunPosition.transform.position.z + 200f);
            TagSun = false; //1초마다 (생성될 때마다) 닿이지 않았음으로 초기화
